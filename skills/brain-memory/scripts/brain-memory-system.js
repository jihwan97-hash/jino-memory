...
function getNewJsonlFiles(state) {
  ...
  // Scan for .jsonl files in a directory (recursive)
    ...
    try {
      for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
        ...
        if (entry.isDirectory()) {
          ...
          // Deduplicate: backup dir may have copies of live session files
  ...
  // Primary: live OpenClaw session files
  scan(AGENTS_DIR, AGENTS_DIR);
  // Fallback: backed-up session files (persisted across restarts)
  scan(SESSIONS_BACKUP_DIR, SESSIONS_BACKUP_DIR);
