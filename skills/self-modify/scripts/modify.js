...
function resolvePath(fileArg) {
  ...
  // brain-memory/* files (daily summaries, reflections, email summaries)
  if (fileArg.startsWith('brain-memory/') && fileArg.endsWith('.md')) {
    const fullPath = path.join(WORKSPACE, fileArg);
    return { maxTokens: 800, path: fullPath, key: fileArg };
  }
